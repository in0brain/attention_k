...
## Sprint 3C-2：MLP Readout Direction Analysis and Donor-Free Nudge（本轮）

目标：3C-1 把选择性、低 harm 的答案读出写定位到 MLP output。本轮问：这个写是不是一个稳定方向？是否与 gold-vs-wrong unembedding 对齐？能否脱离 correct donor 变成 **donor-free** 方向保留效果？**不是** full 3C / 2000 / 训练 / 可部署 steering，全程 teacher-forced（+first-step）proxy。默认四标记 false。

复用与设置：复用 34 对 3C-0-Fix pair（不重采样）。抓 correct/wrong 在答案读出位的 MLP output，算 correct−wrong delta（layers {20,24}）。donor-free 方向用 **leave-one-out** 构造（mean_delta / pc1_delta / negative / shuffled，均剔除评估 pair），外加 gold_unembed（eval-only，per-pair）、random、zero。nudge：`mlp_output[readout] += α·‖mlp_output[readout]‖·unit(direction)`，α∈{0.05,0.1,0.2,0.4}，1904 cell。指标沿用 corrected answer-sequence clean_direction + first-step（仅 prefix）generation-style proxy。

新增：`src/recover_attention/mlp_readout_direction.py`（extract_mlp_readout / compute_correct_wrong_delta / normalize_direction / mean_direction / pca_direction / unembedding_answer_direction / register_mlp_direction_nudge / sequence_logprob_with_direction_nudge / first_step_logits_with_nudge / geometry & alignment helpers）、`scripts/sprint_3C_2_mlp_readout_direction_analysis.py`、`tests/test_mlp_readout_direction.py`（9 测试）。

核心结果——**mixed：机制发现清晰，但没有 robust 的 donor-free 方向**：

- geometry：correct−wrong MLP delta 有非常稳定的主轴（PC1 LOO cosine 0.99@L20 / 0.98@L24；PC1 explained variance ~0.12–0.13，约 4× 均匀）。存在共享方向。
- unembedding 对齐：**L24** 上 per-pair delta 与 gold-vs-wrong unembedding 方向对齐，cosine +0.091 CI95 [+0.064,+0.117]（稳定为正）；L20 ~0。即 L24 的 MLP 写（弱但稳定地）指向 gold token 的 unembedding。
- gold-unembedding 方向（**eval-only**，用 gold）：强、低 harm、first-step 存活——L24 clean +0.34→+2.93（α 越大越强，gold 升 wrong 降），harm ≤0.12，每档 α 稳定；比 random 高 +0.93 CI95 [+0.73,+1.13]。是 oracle 上界，不可部署。
- donor-free mean_delta 方向：L24 上相对 no-op 稳定为正（clean +0.13/+0.09/+0.15/+0.31，harm 0.0），但只**勉强**跑赢 random（+0.062 CI95 [+0.0005,+0.130]）；**不**稳定跑赢自身 negation（+0.062 CI95 [-0.029,+0.166]），first-step 几乎不存活（仅 α0.4 稳定）。L20 失败。PC1 方向不跑赢 random。
- control 注记：`shuffled` 控制在此**退化**——打乱 correct/wrong 配对再求均值会还原同一个 mean 方向（`mean_delta − shuffled ≡ 0`），因此 shuffle 对全局 mean 方向无信息量（只在 per-pair 层面才有意义）；全局方向的有效对照是 random 与 negative。

读法：答案读出位的 MLP 写是一个真实、稳定、低秩的方向，L24 上与 gold token unembedding 对齐——机制上就是「MLP 把 residual 往 gold 答案 token 推」。但有效轴本质上**需要知道 gold 答案**（gold_unembed 有效；gold-free 的 mean/PC1 方向只勉强有效、不 robust）。因此**没有**可部署的 donor-free steering handle。这回到项目反复出现的主题：信号更适合 attribution/detection（读出 MLP 被推向哪个答案 token），而非盲 steering 控制。

检查：定向 pytest 9 passed；full pytest 642 passed, 2 skipped。review gate 17 问全答，verdict 四项 false，判定「MLP readout 带因果信息、与 gold unembedding 对齐，但当前提取不足以支撑干净的 donor-free steering」。
