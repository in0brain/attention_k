...
5. 本地模型 D:/models/Qwen2.5-7B-Instruct 是否可用；
...
必报 num_pairs；若 < 20 对，如实记录 insufficient_pairs 并把 Stage 5
降级为 available-pairs-only，不要强行扩采样。
```

### Stage 5：3C-1 位点迁移验证（Q3）
...
module_patch_fidelity_report.json
