...
```text
1. REQUIRED_FIELDS["masked_question"] 与 interface 的 required_fields marker 块一致
   （由 python scripts/sync_interface_fields.py --check 验证）。
2. label_schema.md 只指向 interface、不复制字段表（由 test_interface_consistency.py 验证）。
3. mask_backend = unit_mask_v0，mask_strategy = replace_each_span。
4. 没有任何文件仍把 Masked Question 定义为旧 span-level。
5. validate_masked_question_record 拒绝 span_id / span_text / span_type。
```
...
10. python scripts/sync_interface_fields.py --check 是否通过（interface marker 与 REQUIRED_FIELDS 一致）。
...
```text
- unit 结构字段（id / unit_id / unit_scope / group_type / span_ids / spans /
  original_question）：从聚合组内任一来源 1E 记录复制。
- 三个 source id 列表 + semantic_sources：由聚合产生，每个来源 1E 记录贡献一项。
- 其余（masked_id / masked_question / mask_token / mask_backend / mask_strategy）：由 1F 生成。
```
...
历史保留的详细检查项（供参考，非权威，权威以 schemas.py 为准）：
