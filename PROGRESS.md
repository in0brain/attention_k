...
当前主线停在 Sprint 4D-0：H1 fabricated-identifier 任务的数据与标签设计已完成。
...
当前 Sprint 4 总结：finite-label cyber MCQ 上 F1/F4 未提供相对 F5 的稳定增量；H1 fabricated-identifier 已具备设计与数据基座。下一步只能先做 4D-1 emission/base-rate smoke，不得直接进入内部特征工程或 intervention。
...
Sprint 4D-0 已运行 ontology 下载、H1 dataset 构造与全量 pytest；详细命令、检查和 artifact 清单归档于 Sprint 4 history/manifest。
...
- Sprint 4D-0 只证明 H1 数据/标签设计存在；未证明模型会稳定输出 id，未证明 fabrication base rate 落在可用区间。

## 6. 下一步建议

建议下一步起草并执行 Sprint 4D-1：只做 H1 smoke generation 的 id emission rate 与 fabrication base rate 实测。

边界：不要自动开始 F1-F4 内部特征、probe training、attention steering、site-transfer patching、2000-scale rerun 或 hallucination-reduction claim；必须先有明确 task card 或用户指令。
