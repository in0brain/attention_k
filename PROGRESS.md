...
| G3 | ≤20 prompt 真实 model-in-loop smoke 通过 | **未过（v2.3 后失效，待重挣）** |
...
预注册版本：v2.1（`b3ef9228…`，07-15）→ v2.2（`ffa722e8…`，07-16）→ **v2.3（`16fa43db…`，07-16，当前）**。两次修订都在 Stage 0 之前，理由与正当性记录在 preregistration.md 顶部；旧版 hash 与实现保留在 commit `57b7ae9` / `f93877b`，不覆盖。

- **v2.2**：只改 §7 融合协议（dense 拆成 F5 / H 两个独立 block，三块等权）。修的是 v2.1 内部不一致——它防住 sparse 压制 dense，却放任 3584 维 H 压制 14 维 F5，使 `AUROC(O+H) − AUROC(O)` 混入"融合是否破坏输出信号"这个工程混杂。
- **v2.3**：只改 **MCQ 臂**（H1 的 480 prompt × K=6 经 sizing 判定充分，不变）。修的是一处 **pre-Stage-0 事实错配**：§7 曾断言"CyberMetric prompt 要求先简短推理再给字母"，但实测 `has_reasoning_text = 0/240`、输出仅一个字母。新增 §7.1 task-specific semantic output canonicalization（MCQ 的字母是**指针**，语义实现 = 被选中选项的文本）与 §7.2 adequate precision（可证伪判据）；MCQ confirmatory = fresh 1760 题（2000 − 已烧 240）。依据见 `docs/paper/mcq_asset_audit_and_v2.3_rationale.md`。

每次修订都会让上一版的 G3 **自动失效**——G3 校验含 `prereg_sha256` 比对，无需手动作废。
...
**当前阶段 = W0.5-C：MCQ 侧实现 + 重挣 v2.3 的 G3。唯一目标是拿回 G3，不启动任何正式实验。**

链路（任务卡 §0.1）：MCQ adapter（Stage B，确定性转换，不重采样）→ selected-option semantic canonicalization（§7.1）→ teacher-forced hidden re-forward（Stage C）→ v2.3 ladder / O / H / O+H（Stage D）→ ≤20 pilot prompt smoke → 新 G3。
...
2. Stage 0 其余执行路径（见 §5 缺口）：H1 全量生成、正式 ladder/gate/increment/rq2、Llama 复核。

**新 G3 通过前不得**：跑 fresh 1760 的任何正式结果、算 confirmatory AUROC、跑正式 Δ_H CI、启动 H1 2880 全量、把 pilot smoke 数值写成发现。新 G3 通过后仍需 G1 才能启动 Stage 0。
