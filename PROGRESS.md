...
当前主线停在 Sprint 4D-1：H1 fabricated-identifier emission/base-rate smoke 已完成，`h1_gate_passed=true`。
...
当前 Sprint 4 总结：finite-label cyber MCQ 上 F1/F4 未提供相对 F5 的稳定增量；H1 fabricated-identifier 已通过 emission/base-rate smoke gate，具备进入 4D-2 full H1 generation + H1-F5 baseline 的条件。仍不得直接进入 intervention 或 hallucination-reduction claim。
...
Sprint 4D-1 已运行 H1 smoke generation、targeted pytest 与 full pytest；详细命令、检查和 artifact 清单归档于 Sprint 4 history/manifest。
...
建议下一步起草并执行 Sprint 4D-2：full H1 generation + H1-F5 baseline。
